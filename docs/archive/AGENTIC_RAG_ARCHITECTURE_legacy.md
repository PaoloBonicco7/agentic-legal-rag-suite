...
Obiettivo: progettare un’architettura **incrementale e misurabile** che migliori progressivamente l’accuratezza del 
retrieval (e quindi delle risposte), sfruttando la ricchezza dell’HTML per costruire metadati e controllare meglio la selezione del contesto.
...
- **Incrementalità:** una modifica per volta, con confronto diretto rispetto alla baseline.
- **Misurabilità:** metriche focalizzate su retrieval + qualità risposta.
- **Sfruttamento dell’HTML:** metadati strutturali e riferimenti espliciti come “segnali” per retrieval e ranking.
- **Trade-off accuratezza/manutenzione:** ogni step deve motivare costo e complessità aggiunta.

---

### 4) Architettura a step (dal Naive RAG a retrieval avanzato)

#### Step 0 — Dataset e protocolli di valutazione (fondamenta)
- Definizione del set di domande
  - Presente un file `questions.csv`con 100 domande a risposta multipla, suddivisi in 4 livelli di complessità.
- Metriche minime:
