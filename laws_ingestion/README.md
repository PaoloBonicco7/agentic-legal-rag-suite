...
Pipeline deterministica (no LLM) per trasformare un corpus di leggi regionali in HTML (`data/leggi-html/*.html`) in un dataset **RAG-ready** (JSONL flat) con:
...
> Obiettivo: partire da HTML “navigazionali” e ottenere un dataset indicizzabile (vector store/BM25) + metadati utili (status, connessioni).
...
## CLI
```bash
python -m laws_ingestion ingest --html-dir data/leggi-html --out-dir data/laws_dataset
python -m laws_ingestion qa --out-dir data/laws_dataset
python -m laws_ingestion debug-law --html-dir data/leggi-html --law-id vda:lr:1991-09-05:44
```
